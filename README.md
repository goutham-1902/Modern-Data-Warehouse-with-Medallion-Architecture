...
1. 1.  **Extract:** Load CSV data into SQL Server's Bronze Layer.
1.     
1. 2.  **Transform:** Cleanse and merge CRM and ERP data in Silver Layer.
1.     
1. 3.  **Load:** Model structured Fact and Dimension tables in Gold Layer.
1.     
1. 4.  **Analyze:** Use Jupyter Notebook to query and visualize insights directly from Gold Layer.
1.     
...
`docker run \   -e "ACCEPT_EULA=1" \   -e "MSSQL_SA_PASSWORD=MyStrongPass123" \   -e "MSSQL_PID=Developer" \   -p 1433:1433 \   -v ~/projects/my_dwh_prj:/my_dwh_prj \   --name sqlserver \   -d mcr.microsoft.com/azure-sql-edge`
...
`python -m venv venv source venv/bin/activate  # For Windows: venv\Scripts\activate  pip install pandas pyodbc jupyter notebook`
...
1. 1.  Run SQL scripts for schema creation and ETL from the repository in SSMS or VS Code.
1.     
1. 2.  Open the provided Jupyter notebook and execute cells to:
1.     
1.     * *   Connect to the SQL Server
1.     *     
1.     * *   Run queries on the Gold Layer
1.     *     
1.     * *   Visualize business insights
1.     *     
